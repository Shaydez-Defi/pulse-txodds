...
export function PressureHeatmap({ zones }: { zones: Zone[] }) {
  ...
  return (
    <div>
      <h3 className="font-display mb-4 text-lg font-semibold">Pressure heatmap</h3>
      <div className="relative aspect-[68/105] w-full overflow-hidden rounded-2xl border border-white/8 bg-[#0a120e]">
        <svg viewBox="0 0 68 105" className="h-full w-full">
          ...
          {active.map((zone, i) => (
            <circle
              ...
              fill={`rgba(139, 74, 58, ${zone.intensity * 0.45})`}
            />
          ))}
        </svg>
      </div>
